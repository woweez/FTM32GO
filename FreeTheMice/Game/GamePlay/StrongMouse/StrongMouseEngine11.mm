...
-(id) init
{
    if( (self=[super init])) {
        ...
        pushButtonSprite.position = ccp(43,349);
        if (![FTMUtil sharedInstance].isRetinaDisplay) {
    ...
    for(int i=0;i<4;i++){
        ...
        if(iceQubeCount[i]!=0){
            if(iceQubeCount[i]<230){
                ...
                if (iceQubeCount[i] > 230 && iceQubeCount[i] < 233) {
...
-(void)level05{
    ...
    if (gameFunc.honeyPotCount2 == 30 && pushButtonSprite.tag != 10) {
        [self resetRedPushBtn:ccp(43,332)];
    }
}
-(void)level01{
    if(firstRunningChe){
