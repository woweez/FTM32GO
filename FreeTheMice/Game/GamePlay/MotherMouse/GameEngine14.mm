...
//        [self addChild:gateSprite z:9];
        [self addGateImageAndAnimation:ccp(970, 275)];
...
-(void)iceCubeCollision{
   ...
   for(int i=0;i<3;i++){
        if(![FTMUtil sharedInstance].isInvincibilityOn &&  hx-iValue>iceQubeSprite[i].position.x-60 &&hx-iValue<iceQubeSprite[i].position.x+20 &&hy > iceQubeSprite[i].position.y-30 &&hy<iceQubeSprite[i].position.y+50 &&!gameFunc.trappedChe && iceQubeSprite[i].visible){
    ...
    if(![FTMUtil sharedInstance].isInvincibilityOn && hx>xx-30 && hx< xx+20 &&hy > yy-30  &&!gameFunc.trappedChe&&!forwardChe){
        gameFunc.trappedChe=YES;
        trappedTypeValue=3;
    }else if(![FTMUtil sharedInstance].isInvincibilityOn && hx-iValue > xx-60 &&hx-iValue<xx-20 &&hy > yy-30  &&!gameFunc.trappedChe&&forwardChe){
        gameFunc.trappedChe=YES;
        trappedTypeValue=3;
    }
    ...
    if(hx-iValue>720 &&hx-iValue<800 &&hy > 570 && hy<600 &&gameFunc.moveCount2==0){
        if(![FTMUtil sharedInstance].isInvincibilityOn && eleAtlas.position.x==820){
            trappedTypeValue=5;
            gameFunc.trappedChe=YES;
        }else{
        ...
        if(![FTMUtil sharedInstance].isInvincibilityOn && eleAtlas.position.x==430){
            trappedTypeValue=5;
            gameFunc.trappedChe=YES;
        }else{
    ...
    if(![FTMUtil sharedInstance].isInvincibilityOn && hx-iValue>465 &&hx-iValue<630 && hy==266&&!gameFunc.trappedChe){
        trappedTypeValue=4;
        gameFunc.trappedChe=YES;
    }
...
-(void)level05{
    ...
    if(gateCount>=1) {
        if (gateSprite.tag != 1010) {
            gateSprite.tag = 1010;
            [self playDoorAnimation];
        }
        gateCount+=0.1;
...
-(void)hotSmokingFunc{
    ...
    if(![FTMUtil sharedInstance].isInvincibilityOn && heroSprite.position.x-iValue<100 && heroSprite.position.y>250&&heroSprite.position.y<=435&&!heroTrappedChe&&gameFunc.switchCount==0){
        gameFunc.trappedChe=YES;
        trappedTypeValue=2;
    }
