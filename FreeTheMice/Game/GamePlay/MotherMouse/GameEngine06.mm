...
-(id) init
{
    if( (self=[super init])) {
        ...
        if (![FTMUtil sharedInstance].isRetinaDisplay) {
            pSprite.scaleY = NON_RETINA_SCALE;
            pSprite.scaleX = 0.55;
        }else{
            pSprite.scaleX=1.1;
        }
        ...
        if (![FTMUtil sharedInstance].isRetinaDisplay) {
            potSprite.scale = NON_RETINA_SCALE;
        }
        ...
        if (![FTMUtil sharedInstance].isRetinaDisplay) {
            potSprite.scale = NON_RETINA_SCALE/2;
        }else{
            potSprite.scale = 0.5;
        }
        [self addChild:potSprite z:9];
        ...
        if (![FTMUtil sharedInstance].isRetinaDisplay) {
            tileMove2.scale = NON_RETINA_SCALE;
        }
...
//                 if(catMovementValue>=(screenMovementFindValue<2?60:500)){
    ...
    if(jumpingChe){
        ...
        if(hx-iValue>catRunSprite.position.x-100 &&hx-iValue<catRunSprite.position.x+60 &&hy > catRunSprite.position.y-30 &&hy<catRunSprite.position.y+10 &&!gameFunc.
                trappedChe && ![FTMUtil sharedInstance].isInvincibilityOn){
            catKnockedOut = YES;
            heroTrappedChe = YES;
