...
-(id) init
{
    if( (self=[super init])) {
        ...
        for(int i=0;i<3;i++){
            switchAtlas[i] = [[CCLabelAtlas labelWithString:@"0" charMapFile:@"switch.png" itemWidth:switchWidth itemHeight:switchHeight startCharMap:'0'] retain];
            if(i==0)
        ...
        int bubbleWidth = [FTMUtil sharedInstance].isRetinaDisplay ? 25: 50;
        int bubbleHeight = [FTMUtil sharedInstance].isRetinaDisplay ? 25: 50;
        
        for(int i=0;i<2;i++){
            waterBubbleAtlas[i] = [[CCLabelAtlas labelWithString:@"4" charMapFile:@"water_bubble.png" itemWidth:bubbleWidth itemHeight:bubbleHeight startCharMap:'0'] retain];
            waterBubbleAtlas[i].position=ccp(-430,50);
            if (![FTMUtil sharedInstance].isRetinaDisplay) {
                waterBubbleAtlas[i].scaleX = 1.25;
                waterBubbleAtlas[i].scaleY = 0.25;
            }else{
        ...
        if (![FTMUtil sharedInstance].isRetinaDisplay) {
            bridgeSprite.scaleY = NON_RETINA_SCALE;
            bridgeSprite.scaleX = 0.505;
        }else{
        ...
        if (![FTMUtil sharedInstance].isRetinaDisplay) {
            combSprite.scale = NON_RETINA_SCALE;
        }
        [self addChild:combSprite z:1];
...
-(void)iceCubeCollision{
    ...
    for(int i=0;i<4;i++){
        if(![FTMUtil sharedInstance].isInvincibilityOn && hx-iValue>iceQubePos[i][0]-90 &&hx-iValue<iceQubePos[i][0]-30 &&hy > iceQubePos[i][1]-30 &&hy<iceQubePos[i][1]+20 &&!gameFunc.trappedChe){
             gameFunc.trappedChe=YES;
             mouseTrappedPosValue=iceQubeCount[i];
        }else if(![FTMUtil sharedInstance].isInvincibilityOn && hx-iValue>iceQubePos2[i][0]-90 &&hx-iValue<iceQubePos2[i][0]-30 &&hy > iceQubePos2[i][1]-30 &&hy<iceQubePos2[i][1]+20 &&!gameFunc.trappedChe){
             gameFunc.trappedChe=YES;
             mouseTrappedPosValue=iceQubeCount2[i];
        }
    }
    if(![FTMUtil sharedInstance].isInvincibilityOn && levelFloddedValue>heroSprite.position.y+20&&!waterTrappedChe){
        gameFunc.trappedChe=YES;
        waterTrappedChe=YES;
    }
