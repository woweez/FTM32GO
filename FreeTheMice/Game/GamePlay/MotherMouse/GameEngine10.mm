...
-(void)iceCubeCollision{
    ...
    if(![FTMUtil sharedInstance].isInvincibilityOn && gameFunc.switchCount==0&&pieScaleCount>40){
        if(hx-iValue>850&&hy<250){
            gameFunc.trappedChe=YES;
        }
        if(![FTMUtil sharedInstance].isInvincibilityOn && hx-iValue>750&&hy>250&&hy<350&&pieScaleCount==230){
            gameFunc.trappedChe=YES;
        }
    }
}
...
//             NSLog(@"Object width and height==== %d %d %d", gameFunc.objectWidth,gameFunc.objectHeight,gameFunc.sideValueForObject);
            ...
            heroTrappedSprite.scale = MAMA_MIST_TRAPPED;
    ...
    if(mouseWinChe){
        ...
        if(heroWinCount==15){
            ...
            if(!forwardChe)
                ...
                heroWinSprite.position = ccp(950, platformY+5);
            heroWinSprite.scale = MAMA_NORMAL_TRAPPED;
